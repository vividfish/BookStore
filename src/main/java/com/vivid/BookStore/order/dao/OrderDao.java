...
public interface OrderDao {
	 ...
	 * @throws SQLException
	 */
	public Page<Order> findByUser(String uid, int pc);
	...
	Page<Order> findByCriteria(Order order, int pc);
	...
	// * 4. 得到beanList，即当前页记录
	// */
	// sql = "select * from t_order" + whereSql + " order by ordertime desc
	// limit ?,?";
	// params.add((pc - 1) * ps);// 当前页首行记录的下标
	// params.add(ps);// 一共查询几行，就是每页记录数
	//
	// List<Order> beanList = qr.query(sql, new
	// BeanListHandler<Order>(Order.class), params.toArray());
	 ...
	 * @return
	 */
	List<OrderItem> toOrderItemList(List<Map<String, Object>> mapList);
	 ...
	 */
	OrderItem toOrderItem(Map<String, Object> map);
